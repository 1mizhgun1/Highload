...
Такой подход обеспечивает равномерное распределение данных по шардам, что повышает отказоустойчивость. Применим шардирование для таблицы Page, так как она будет очень тяжелой, а также для таблиц Attach и ACL, так как в них будет много записей.
...
Помимо отказоустойчивости и балансирвоки нагрузки, такой подход позволяет распараллелить поисковые запросы, что значительно ускорит поиск.
